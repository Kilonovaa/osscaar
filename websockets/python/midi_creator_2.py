...
verticalKernel = np.array(
    [0.45, 0.7, 0.9, 1.0, 1.0, 1.0, 0.9, 0.7, 0.45], dtype=float)
verticalKernel *= 0.58
...
def addNoteAtIndex(midiFiles, timeBetweenNotes: float,
    ...
    midiFiles[noteIndex % len(midiFiles)].addNote(track=balanceChannel, channel=balanceChannel,
                                                  pitch=pitch, time=noteStart, duration=noteDuration, volume=volume)
...
def postProcessing(midiFiles, midiLength: float, timeBetweenNotes: float,
    ...
    volumeZero2Duration = timeBetweenNotes * len(midiFiles) - (
        volumeZeroDuration + volumeSlopeUpDuration + volumeMaxDuration + volumeSlopeDownDuration)
    assert (volumeZero2Duration >= 0.0)

    for i in range(len(midiFiles)):
        for ch in range(16):
            balance = getBalanceFromChannel(ch)
            midiFiles[i].addControllerEvent(
                track=ch, channel=ch, time=0, controller_number=8, parameter=balance)
            # tempo is in beats per minute
            midiFiles[i].addTempo(track=ch, time=0, tempo=tempo)

            midiFiles[i].addControllerEvent(
                track=ch, channel=ch, time=0, controller_number=7, parameter=maxVolume)
            midiFiles[i].addControllerEvent(
                track=ch, channel=ch, time=0, controller_number=11, parameter=0)
            j = float(i) * timeBetweenNotes
            while j <= midiLength:

                midiFiles[i].addControllerEvent(
                    track=ch, channel=ch, time=j, controller_number=11, parameter=0)
                j += volumeZeroDuration
                if volumeSlopeUpDuration > 0.0:
                    for k in np.arange(0, maxExpression, incVolumeResolution, dtype=float):
                        midiFiles[i].addControllerEvent(track=ch, channel=ch, time=j + (float(
                            k) / maxExpression) * volumeSlopeUpDuration, controller_number=11, parameter=max(min(round(k), maxExpression), 0))
                j += volumeSlopeUpDuration
                midiFiles[i].addControllerEvent(
                    track=ch, channel=ch, time=j, controller_number=11, parameter=maxExpression)
                j += volumeMaxDuration
                if volumeSlopeDownDuration > 0.0:
                    for k in np.arange(0, maxExpression, decVolumeResolution, dtype=float):
                        midiFiles[i].addControllerEvent(track=ch, channel=ch, time=j + (float(k) / maxExpression) * volumeSlopeDownDuration,
                                                        controller_number=11, parameter=max(min(maxExpression - round(k), maxExpression), 0))
...
def addNotesFromFrame(midiFiles, frame: np.ndarray, lowerHsv: np.ndarray, upperHsv: np.ndarray,
    ...
    if (lowerHsv[0] > upperHsv[0]):
        mask1 = cv2.inRange(frame, lowerHsv, np.array(
            [179, upperHsv[1], upperHsv[2]]))
        mask2 = cv2.inRange(frame, np.array(
            [0, lowerHsv[1], lowerHsv[2]]), upperHsv)
    ...
    callback(lastCompletion,
             "Calculating pitch - balance frequency matrix", nrFrames)
    height, width, _ = frame.shape
    frArray = np.zeros((128, 16), dtype=float)
    maskArray = np.zeros((16), dtype=float)
    frame = cv2.convertScaleAbs(frame, alpha=float(
        maxPitch - minPitch) / 255.0, beta=minPitch)

    for i in range(height):
        for j in range(width):
            pitch = frame[i][j][2]
            balanceIndex = getChannelFromBalance(
                round(float(j) / (width-1) * 127.0))
            frArray[pitch][balanceIndex] += mask[i][j]
            maskArray[balanceIndex] += mask[i][j]

    callback(lastCompletion,
             "Normalizing frequency matrix by mask per-balance density", nrFrames)
    ...
    frArray = np.apply_along_axis(
        frArrayModifyVertical, 0, frArray)  # vertical
    frArray = np.apply_along_axis(
        frArrayModifyHorizontal, 1, frArray)  # orizontal
    ...
    for i in currentChord:
        ...
        for j in range(0, 16, 1):
            if frArray[i][j] > threshVolume:
                volume = max(
                    min(round(frArray[i][j] - i / 8.0), maxVolume), minVolume)
                addNoteAtIndex(midiFiles, timeBetweenNotes,
                               noteIndex, i, volume, j)
...
def getMidisFromVideo(videoPath: str, idString: str, myCallback):
    ...
    bassMidiFiles = []
    for i in range(15):
        violinMidiFiles.append(
            MIDIFile(numTracks=16, adjust_origin=False, eventtime_is_ticks=False))
        pianoMidiFiles.append(
            MIDIFile(numTracks=16, adjust_origin=False, eventtime_is_ticks=False))
        harpMidiFiles.append(
            MIDIFile(numTracks=16, adjust_origin=False, eventtime_is_ticks=False))
        guitarMidiFiles.append(
            MIDIFile(numTracks=16, adjust_origin=False, eventtime_is_ticks=False))
        fluteMidiFiles.append(
            MIDIFile(numTracks=16, adjust_origin=False, eventtime_is_ticks=False))
        bassMidiFiles.append(
            MIDIFile(numTracks=16, adjust_origin=False, eventtime_is_ticks=False))
    ...
    callback(lastCompletion, "Processing frame " +
             str(frameNumber) + " of " + str(nrFrames), nrFrames)
    while frameExists:
        ...
        if index != lastIndex:
            ...
            scaleFactor = min(float(videoNewSize) / float(height),
                              float(videoNewSize) / float(width))
            if scaleFactor < 1.0:
                hsvFrame = cv2.resize(
                    hsvFrame, (round(width * scaleFactor), round(height * scaleFactor)))
            addNotesFromFrame(violinMidiFiles, hsvFrame, lowerViolin,
                              upperViolin, timeBetweenNotes, lastIndex)
            addNotesFromFrame(pianoMidiFiles, hsvFrame, lowerPiano,
                              upperPiano, timeBetweenNotes, lastIndex)
            addNotesFromFrame(pianoMidiFiles, hsvFrame, lowerPiano2, upperPiano2,
                              timeBetweenNotes, lastIndex, minVolume=40, maxVolume=110, threshVolume=45)
            addNotesFromFrame(harpMidiFiles, hsvFrame, lowerHarp,
                              upperHarp, timeBetweenNotes, lastIndex)
            addNotesFromFrame(guitarMidiFiles, hsvFrame, lowerGuitar,
                              upperGuitar, timeBetweenNotes, lastIndex)
            addNotesFromFrame(fluteMidiFiles, hsvFrame, lowerFlute,
                              upperFlute, timeBetweenNotes, lastIndex)
            addNotesFromFrame(bassMidiFiles, hsvFrame, lowerBass,
                              upperBass, timeBetweenNotes, lastIndex)
    ...
    postProcessing(violinMidiFiles, videoLength, timeBetweenNotes,
                   60, 0.3, 0.8, 2.2, 0.8, maxVolume=96)
    postProcessing(pianoMidiFiles, videoLength,
                   timeBetweenNotes, 60, 0.3, 0.8, 2.2, 0.8)
    postProcessing(harpMidiFiles, videoLength,
                   timeBetweenNotes, 60, 0.3, 0.8, 2.2, 0.8)
    postProcessing(guitarMidiFiles, videoLength,
                   timeBetweenNotes, 60, 0.3, 0.8, 2.2, 0.8)
    postProcessing(fluteMidiFiles, videoLength,
                   timeBetweenNotes, 60, 0.3, 0.8, 2.2, 0.8)
    postProcessing(bassMidiFiles, videoLength,
                   timeBetweenNotes, 60, 0.3, 0.8, 2.2, 0.8)
    ...
    if len(violinMidiFiles) > 1:
        ...
        for index in range(1, len(violinMidiFiles)):
            callback(lastCompletion + float(index) / len(violinMidiFiles) /
                     12.0, "Creating violin .wav number " + str(index), nrFrames)
            with open(violinMidiPath, "wb") as outf:
    ...
    if len(pianoMidiFiles) > 1:
        ...
        for index in range(1, len(pianoMidiFiles)):
            callback(lastCompletion + float(index) / len(pianoMidiFiles) /
                     12.0, "Creating piano .wav number " + str(index), nrFrames)
            with open(pianoMidiPath, "wb") as outf:
    ...
    if len(harpMidiFiles) > 1:
        ...
        for index in range(1, len(harpMidiFiles)):
            callback(lastCompletion + float(index) / len(harpMidiFiles) /
                     12.0, "Creating harp .wav number " + str(index), nrFrames)
            with open(harpMidiPath, "wb") as outf:
    ...
    if len(guitarMidiFiles) > 1:
        ...
        for index in range(1, len(guitarMidiFiles)):
            callback(lastCompletion + float(index) / len(guitarMidiFiles) /
                     12.0, "Creating guitar .wav number " + str(index), nrFrames)
            with open(guitarMidiPath, "wb") as outf:
    ...
    if len(fluteMidiFiles) > 1:
        ...
        for index in range(1, len(fluteMidiFiles)):
            callback(lastCompletion + float(index) / len(fluteMidiFiles) /
                     12.0, "Creating flute .wav number " + str(index), nrFrames)
            with open(fluteMidiPath, "wb") as outf:
    ...
    if len(bassMidiFiles) > 1:
        ...
        for index in range(1, len(bassMidiFiles)):
            callback(lastCompletion + float(index) / len(bassMidiFiles) /
                     12.0, "Creating bass .wav number " + str(index), nrFrames)
            with open(bassMidiPath, "wb") as outf:
    ...
    segmAll = segmViolin.overlay(segmPiano).overlay(segmHarp).overlay(
        segmGuitar).overlay(segmFlute).overlay(segmBass)
