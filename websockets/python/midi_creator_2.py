...
def getMidisFromVideo(videoPath: str, idString: str, callback):
    ...
    videoNewSize = 256
    videoLength = float(cap.get(cv2.CAP_PROP_FRAME_COUNT)
    ...
    while frameExists:
        ...
        if index != lastIndex:
            ...
            addNotesFromFrame(pianoMidiFiles, hsvFrame, lowerPiano2,
                              upperPiano2, timeBetweenNotes, lastIndex)
    ...
    postProcessing(violinMidiFiles, videoLength + 3.0,
                   timeBetweenNotes, 60, 0.3, 0.8, 3.5, 0.8, callback)
    postProcessing(pianoMidiFiles, videoLength + 3.0,
                   timeBetweenNotes, 60, 0.3, 0.8, 3.5, 0.8, callback)
    postProcessing(harpMidiFiles, videoLength + 3.0,
                   timeBetweenNotes, 60, 0.3, 0.8, 3.5, 0.8, callback)
    postProcessing(guitarMidiFiles, videoLength + 3.0,
                   timeBetweenNotes, 60, 0.3, 0.8, 3.5, 0.8, callback)
    postProcessing(fluteMidiFiles, videoLength + 3.0,
                   timeBetweenNotes, 60, 0.3, 0.8, 3.5, 0.8, callback)
    postProcessing(bassMidiFiles, videoLength + 3.0,
                   timeBetweenNotes, 60, 0.3, 0.8, 3.5, 0.8, callback)
    ...
    if len(violinMidiFiles) > 1:
        ...
        for index in range(1, len(violinMidiFiles)):
            callback(lastCompletion + float(index) /
                     len(violinMidiFiles) / 12.0)
            with open(violinMidiPath, "wb") as outf:
