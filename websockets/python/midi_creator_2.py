...
def postProcessing(midiFiles, midiLength: float, timeBetweenNotes: float,
    ...
    volumeMaxDuration = timeBetweenNotes * len(midiFiles) - (
        volumeZeroDuration + volumeSlopeUpDuration + volumeSlopeDownDuration + volumeZero2Duration)
...
def getMidisFromVideo(cap: cv2.VideoCapture, timeBetweenNotes: float, callback):
    ...
    while frameExists:
        ...
        if index != lastIndex:
            ...
            if scaleFactor < 1.0:
                frame = cv2.resize(
                    frame, (round(width * scaleFactor), round(height * scaleFactor)))
    ...
    postProcessing(midiFiles, videoLength + 3.0,
                   timeBetweenNotes, 60, 0.30, 1.0, 0.05, 0.05)
    return midiFiles
